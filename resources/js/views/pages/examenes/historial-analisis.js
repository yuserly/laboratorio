    ...
    methods: {
        ...
        formSubmit() { 
            ...
            if (!this.$v.form.$invalid) {
                this.axios
                    .get(`/api/historialAnalisisEmitidas/${this.form.rut}`)
                    .then(res => {
                        console.log(res);
        ...
        {
            Swal.fire({
                title: "Volver Analizar",
                text: "¿Desea volver analizar este examen?",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#34c38f",
                cancelButtonColor: "#f46a6a",
                confirmButtonText: "Si, Volver Analizar.",
              }).then((result) => {
                if (result.value) {
                  this.axios
                    .get(
                      `/api/volverAnalizar/${data.id_orden_examenes}`
                    )
                    .then((res) => {
                      this.getInicial();
                      Swal.fire({
                        icon: 'success',
                        title: 'Aceptado',
                        text: "Puede volver analizar este examen nuevamente.",
                        timer: 1500,
                        showConfirmButton: false
                    });
                    });
